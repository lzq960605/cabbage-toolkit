...
STEAM_APP_SHADERCACHE_PATH=".local/share/Steam/steamapps/shadercache"
...
class SteamApp(object):
    ...
    __slots__ = (
        ...
        "required_tool_app", "shortcuts_path", "shortcut_id", "shortcut_data"
    )

    def __init__(
            self, name, install_path, prefix_path=None, appid=None,
            required_tool_appid=None, shortcuts_path = None, shortcut_id = None, shortcut_data = None):
        """
        :appid: App's appid
        ...
        :shortcuts_path:  save path for shortcuts.vdf
        :shortcuts_data:  shortcuts data(dict)
        """
    ...
    @classmethod
    def from_appmanifest(cls, path, steam_lib_paths, steam_path):
        """
        ...
        localconfig_path = \
            steam_path / "userdata" / str(steamid3) / "config" / "localconfig.vdf"
        ...
        # save the app info
        shortcuts_path = localconfig_path
        shortcut_id = appid
        shortcut_data = None
        try:
            ...
            if vdf_data['UserLocalConfigStore']['Software']['Valve']['Steam']['apps'].__contains__(str(shortcut_id)):
                shortcut_data = vdf_data['UserLocalConfigStore']['Software']['Valve']['Steam']['apps'][str(shortcut_id)]
        except Exception:
            logger.warning("Skipping malformed localconfig.vdf %s", localconfig_path)
        ...
        return cls(
            ...
            shortcuts_path=shortcuts_path, shortcut_id=shortcut_id, shortcut_data=shortcut_data
        )
...
def get_custom_windows_shortcuts(steam_path):
    ...
    try:
        ...
        origin_vdf_data = vdf.binary_loads(content)
    except IOError:
        logger.info(
    ...
    for shortcut_id, shortcut_data in vdf_data["shortcuts"].items():
        ...
        steam_apps.append(
            SteamApp(
                ...
                shortcuts_path=shortcuts_path, shortcut_id=shortcut_id, shortcut_data=origin_shortcut_data
            )
        )
    ...
    for path in steam_lib_paths:
        ...
        for manifest_path in appmanifest_paths:
            steam_app = SteamApp.from_appmanifest(
                manifest_path, steam_lib_paths=steam_lib_paths, steam_path=steam_path
            )
            if steam_app:
